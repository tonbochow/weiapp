...
class Express {
	...
	preg_match_all("/data\-code\=\"(?P<name>\w+)\"\>(?P<title>.*)\<\/a>/iU", $result, $data);
	$name = array();
    ...
    /**
     * @author wl <wanglei@lingou.com>
     * @lastmodify            2014-8-21
     * 此方法可以用，返回值同上边的方法，无非是验证了一下其他对数组的处理方法。保留，但还是用原生的吧。
     */
    private function expressname2() {
	$result = $this->getcontent("http://www.kuaidi100.com/");
	preg_match_all("/data\-code\=\"(\w+)\"\>(.*)\<\/a>/iU", $result, $data);
	p($data);
	$name = array();
