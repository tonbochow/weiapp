...
//        $this->assign('weixin_userinfo',$this->weixin_userinfo);
    ...
    protected function getWeixinUserInfo() {
        ...
        if (empty($wx_openid)) {//用户未登录
            ...
            if (!isset($get_code) && !isset($get_state)) {//state =1 为snsapi_base模式
            ...
            } else if (isset($get_code) && ($get_state == 1)) {//微信服务器返回code和state  获取access_token凭证
            ...
            } else if (isset($get_code) && ($get_state == 2)) {//snsapi_userinfo模式获取微信用户详细信息
                $userinfo_access_token_info = \Admin\Model\MicroPlatformModel::getSnsAccessTokenByCode(APPID, APPSERCERT, I('get.code'));
                ...
                $memberWeixinModel = D('MemberWeixin');
            ...
            } else if (!isset($get_code) && isset($get_state)) {//snaapi_userinfo 用户不同意授权 仅能获取openid
                $base_access_token_info = \Admin\Model\MicroPlatformModel::getSnsAccessTokenByCode(APPID, APPSERCERT, I('get.code')); //同时获取到微信用户openid
                $wx_openid = $base_access_token_info['openid'];
                //将用户openid插入到数据库中
                $memberWeixinModel = D('MemberWeixin');
